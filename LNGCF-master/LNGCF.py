...
class LNGCF(object):
    def __init__(self, data_config, pretrain_data):
        ...
        *********************************************************
        ...
        """
        if self.alg_type in ['lngcf']:
            self.ua_embeddings, self.ia_embeddings = self._create_lngcf_embed()
    ...
    def _create_lngcf_embed(self):

        def get_weight(num=None):
            canshu = []
            for i in range(1, num + 1):
                a = 1 / (1 + math.pow(math.e, (-1) / i))
                canshu.append(a)
        ...
        all_embeddings = tf.transpose(all_embeddings, perm=[0, 2, 1])

        weight = np.asarray(get_weight(self.n_layers + 1))
        weight = np.expand_dims(weight, axis=1)

        weight2 = tf.convert_to_tensor(weight)

        reshpae_all = tf.reshape(all_embeddings, [(self.n_users + self.n_items) * self.emb_dim, self.n_layers + 1])

        re = tf.reshape(tf.matmul(reshpae_all, tf.cast(weight2, tf.float32)),
                        [(self.n_users + self.n_items), self.emb_dim])

        # all_embeddings=tf.reduce_mean(all_embeddings,axis=1,keepdims=False)
        # all_embeddings = tf.reduce_mean(all_embeddings, axis=1)
        u_g_embeddings, i_g_embeddings = tf.split(re, [self.n_users, self.n_items], 0)
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    if args.pretrain == -1:
        ...
        pretrain_data = None
    model = LNGCF(data_config=config, pretrain_data=pretrain_data)
    ...
    *********************************************************
    ...
    f.write(
        'embed_size=%d, lr=%.4f, layer_size=%s, node_dropout=%s, mess_dropout=%s, regs=%s, adj_type=%s, time=%s\n\t%s\n'
        % (args.embed_size, args.lr, args.layer_size, args.node_dropout, args.mess_dropout, args.regs,
           args.adj_type, time, final_perf))
    f.close()
